...
static NSString* USER_TIME_LINE = @"user_timeline";
static NSString* MENTION_TIME_LINE = @"mention_timeline";
static NSString* HOME_TIME_LINE = @"home_timeline";
...
- (void)userTimelineWithParams:(NSDictionary *)params user:(User *)user completion:(void (^)(NSArray *tweets, NSError *error))completion {
    ...
    NSArray* array = [FileUtil getFromDisk:USER_TIME_LINE];
    if (array) {
        completion([Tweet tweetsWithArray:array], nil);
    }
    else {
        [self GET:getUrl parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [FileUtil saveToDisk:responseObject fileName:USER_TIME_LINE];
...
- (void)mentionsTimelineWithParams:(NSDictionary *)params completion:(void (^)(NSArray *tweets, NSError *error))completion {
    ...
    NSArray* array = [FileUtil getFromDisk:MENTION_TIME_LINE];
    if (array) {
        completion([Tweet tweetsWithArray:array], nil);
    }
    else {
        [self GET:@"1.1/statuses/mentions_timeline.json?include_my_retweet=1" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [FileUtil saveToDisk:responseObject fileName:MENTION_TIME_LINE];
            completion([Tweet tweetsWithArray:responseObject], nil);
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            completion(nil, error);
        }];
...
- (void)homeTimelineWithParams:(NSDictionary *)params completion:(void (^)(NSArray *tweets, NSError *error))completion {
    ...
    NSArray* array = [FileUtil getFromDisk:HOME_TIME_LINE];
    if (array) {
        completion([Tweet tweetsWithArray:array], nil);
    }
    else {
        [self GET:@"1.1/statuses/home_timeline.json?include_my_retweet=1" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [FileUtil saveToDisk:responseObject fileName:HOME_TIME_LINE];
            completion([Tweet tweetsWithArray:responseObject], nil);
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            completion(nil, error);
        }];
